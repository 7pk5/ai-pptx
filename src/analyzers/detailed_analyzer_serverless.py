...
class DetailedPPTAnalyzer:
    """Enhanced PPT analyzer optimized for serverless deployment."""
    ...
    def _analyze_shape(self, shape, shape_idx: int) -> Dict[str, Any]:
        ...
        # Analyze images (simplified for serverless)
        if shape.shape_type == MSO_SHAPE_TYPE.PICTURE:
            shape_data["image_info"] = self._analyze_image_simple(shape)
            if shape_data["image_info"] and "dominant_colors" in shape_data["image_info"]:
    ...
    def _analyze_image_simple(self, shape) -> Dict[str, Any]:
        """Simplified image analysis for serverless deployment."""
        try:
            image = shape.image
            ...
            if PIL_AVAILABLE:
                try:
                    ...
                    # Extract dominant colors if ColorThief is available
                    if COLORTHIEF_AVAILABLE:
                        try:
                            ...
                            image_info["dominant_colors"] = [image_info["average_color"]]
                    
                    # Convert to base64 for web display (limit size)
                    if max(rgb_image.size) > 200:
                        rgb_image.thumbnail((200, 200), Image.Resampling.LANCZOS)
                    ...
                    image_info["error"] = f"PIL processing failed: {str(e)}"
